...
13/11/2017 & 1.1 & Multiple modifications + Test dataset to be implemented\\
26/1/2018 & 1.2 & Correction in \cref{Test-Basic-affine-transformation}\\
\bottomrule
\end{tabularx}
\end{table}
...
\href{https://github.com/slimpotatoes/STEM_Moire_GPA}{\progname{}} software with 
respect to the requirements specified in the SRS document.
...
\item The proposed test plan is focusing on system and unit testing to
  ...
  algorithm errors) and will not catch a code error from a modelling error. A 
physical
  ...
  team on the quality of the test results is required to validate it. If the 
error observed is unreasonably high, an
  error in the code tested might be postulated. At current stage of the 
development, an \enquote{unreasonable error} is difficult to qualify, however a 
relative error greater than 50{\%} of the expected results is not acceptable for 
\progname{}. This limit is excessively high but will be adjusted once the error 
of the GPA algorithm is analytically studied.
...
Alexandre Pofelski is the only member of the test team.
...
Numerous system tests are using analytical functions as inputs and sometimes expect analytical functions as outputs. Those functions need to be implemented into proper format for \progname{} using specific values for the parameters. Dedicated test dataset (inputs and expected outputs) will be explicitly described and stored in the \href{https://github.com/slimpotatoes/STEM_Moire_GPA}{\progname{}} repository once they are designed. 
...
quantitative estimation of strain and rotation.  \wss{I like this
  ...
  going to use?} \an{I fully agree. Will do it later once the tests dataset are 
designed} 
...
\Cref{T_varying-delta-g} might be impossible to properly interpret but the idea 
is to see the effect an improper choice of an unstrained reference. If a non 
uniform deformation field is present in the reference, \progname{} is trying to 
find the best linear fit. A solution is calculated whatever the precision of the 
fit. However, if the quality of the fit is poor, it could be possible that 
\progname{} informs/warns the user of a potential problem in the reference 
chosen.
...
\Cref{black_box_STEM_Moire_GPA} looks more like a validation test than a 
verification test. Nevertheless, if the error in \cref{black_box_STEM_Moire_GPA} 
is unreasonably high, it is possible to consider the presence of a mistake in 
the code somewhere. On the validation side, \cref{black_box_STEM_Moire_GPA} is 
highlighting the accuracy (without noise), and the precision (with noise) of 
\progname{} seen by the end-user (code, user and algorithm errors included). The 
performance is then characterized as the error output function of the noise (or 
of the quality factor of $I_{\mathit{SMH}_{\text{exp}}}$ like the signal on 
noise ratio $\mathit{SNR}=\frac{I_{\mathit{SMH}_\text{exp}}}{N}$). The 
performance characterization can be used to see from which quality factor the 
accuracy and sensitivity of \progname{} is too poor to consider the strain and 
rotation results reliable and warn the user in this case. The performance graph 
...
\item Output: Time to perform \progname{} calculations and plot execution time vs data size.
\end{itemize}
\end{Test}
...
\begin{Test}\normalfont\underline{Test-Black-Box-STEM-Moir{\'e}-GPA-env}
\label{black_box_STEM_Moire_GPA-env}
\begin{itemize}
\item Type: Functional, Dynamical, Black box, Manual
\item Initial State: Various environments (x64 Linux, x64 Windows, x64 Mac OS X) with all user inputs are predefined ($p,M_1,M_2,U,\overrightarrow{q_{n_1,m_1}},\overrightarrow{q_{n_2,m_2}}$) except $I_{\mathit{SMH}_\text{exp}}$ 
...
\item Output: $\varepsilon(\vec{r})^{t}, \omega(\vec{r})^{t}$
	\begin{itemize}
	\item $\forall \vec{r} \in \mathbb{I}, \ E^{\text{\progname{}}}_{\varepsilon\text{ env}}=|\varepsilon(\vec{r})^{t}-\varepsilon(\vec{r})|$
	\item $\forall \vec{r} \in \mathbb{I}, \ E^{\text{\progname{}}}_{\omega\text{ env}}=|\omega(\vec{r})^{t}-\omega(\vec{r})|$
	\end{itemize}
\end{itemize}
\end{Test}

The output of \cref{black_box_STEM_Moire_GPA-env} is not expected to be a 
perfect match but should be matching errors obtained in 
\cref{black_box_STEM_Moire_GPA} (or relatively close to). The errors will be 
then displayed as a function of the environment (Linux, Windows and Mac). The 
relative errors between the different environments can be also considered. 
...
\item Input: dm3 file with pre defined tags (calibration, microscope, high tension, ...)
\item Output: Read the tag from the dm3 file and and test if each tag collected matches with the pre defined tags
\end{itemize}
\end{Test}
...
\item Type: Dynamical, Automated
\item Initial State: Waiting for $M$ user input on $\widetilde{I}_{\mathit{SMH}_{\text{exp}}}$
\item Input: $M$ improper format (such as 1D array, 2D array out of bounds)
...
\enquote{Matrix $G_{\text{uns}}^{\text{exp}}$ non invertible, the
  crystalline wave vectors are linked (collinear). Please choose another 
combination of crystalline wave vectors.}
...
\cref{ext:NR_1} & \cref{black_box_STEM_Moire_GPA-env}\\
...
\toprule
\textbf{Priority} & \textbf{Requirement tested} & \textbf{Associated Tests} \\
\midrule
\endhead
P0 & \cref{ext:R_3}, \cref{ext:R_7}, \cref{ext:R_10} & \cref{aliasing-frequency-simple}, \cref{T_Phase-Extraction-No-Strain}, \cref{T_Phase-Extraction-Known-Strain}, \cref{T_constant-delta-g}, \cref{T_constant-delta-g-with-noise} \\
P1 & \cref{ext:R_12}, \cref{ext:R_13} & \cref{Test-Basic-affine-transformation}, \cref{Test-Error-pixel-affine-transformation}, \cref{Test-Error-q-affine-transformation}, \cref{T_No-2D-strain}, \cref{T_known-constant-2D-strain}, \cref{T_Bad-G_ref}, \cref{T_Bad-Delta-G}\\
P2 & \cref{ext:R_7}, \cref{ext:R_10} & \cref{T_Phase-Extraction-Mask}, \cref{T_varying-delta-g} \\
P3 & \cref{ext:R_2}, \cref{ext:R_6}, \cref{ext:R_9} & \cref{Test-Existence-SMH}, \cref{Test-Format-SMH}, \cref{Test-read-dm3-format}, \cref{Test-Existence-pixel}, \cref{Test-Format-pixel}, \cref{Test-Format-Reference}, \cref{Test-Existence-Reference}, \cref{Test-Existence-Mask}, \cref{Test-Format-Mask}, \cref{Test-Position-Radius-Mask}, \cref{Test-Existence-U}, \cref{Test-Format-U}, \cref{Test-Position-U} \\
P4 & \cref{ext:NR_8}, \cref{ext:NR_9} & \cref{black_box_STEM_Moire_GPA} \\
P5 & \cref{ext:NR_1}, \cref{ext:NR_2} & \cref{black_box_STEM_Moire_GPA-env}, \cref{stress_test_STEM_Moire_GPA} \\
P6 & GUI and other lower priority tasks & N/A \\
\bottomrule
