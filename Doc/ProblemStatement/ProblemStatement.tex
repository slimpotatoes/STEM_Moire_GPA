...
is tilted to respect the ``two beam conditions'' \wss{quotation marks should be
  ``done using the proper LaTeX'' symbols} \an{done} which is not straight forward. Other
...
\wss{in order to can nearly always be replaced by to} \an{done} to
...
\wss{Use a width of 80 columns in your tex file with a hard return ending each
  line.}
\an{done}
